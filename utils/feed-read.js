...
FeedRead.get = function(feed_url, callback) {
  request(feed_url, {timeout: 5000}, function(err, res, body) {
    ...
    if (type == "atom") {
      ...
      return callback(new Error(body.substring(0, 120), "<"+ feed_url +">", res.statusCode));
    }
  });
};
    ...
    callback(null, _.filter(_.map(articles,
      function(art) {
        ...
        var obj = {
          ...
          , description: scrub_html(child_data(art, "description"))
          , enclosure: enclosure ? enclosure.attributes && enclosure.attributes.url ? enclosure.attributes.url : '' : ''
